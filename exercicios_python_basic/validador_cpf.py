...
Problema no algoritmo: 
   Usuário digitar o CPF com pontos e traços
Solução:
    - Utilizar o .replace('trocar caractere', 'substituido na string') --> Pode ser chamado várias vezes. 
                digitos1.replace('.', '') \
                        .replace('-', '') \
                        .replace(' ', '')

    - Utilizar expressão regular (import re)
        r'expressao' --> "r" é utilizado também para colocar expressão regular. 
        [^0-9]       --> tudo que não for um numero
                                    Ex.:
                                                    pattern   troca  string 
                                    digitos1 = re.sub(r'[^0-9]', '', digitos1)

Problema no algoritmo:
    Usuário digitar CPFs como 111.111.111.11 e considerar válido
Solução:
    abcde aaaaa
    746.824.890-70 111.111.111-11 

    - Pegar o primeiro caractere da entrada do usuário * tamanho da entrada
        repeticao_char = digitos1[0] * len(digitos1)
        if digitos1 == repeticao:
            print("Voce digitou um CPF inválido!")

        Se o tamanho da entrada for igual ao primeiro caractere, quer dizer que o usuário digitou numeros repetidos. 
"""
...
repeticao_char = ((digitos1[0] * len(digitos1)) == digitos1)
if repeticao_char == True:
    print("Voce digitou um CPF inválido! (Erro: CPF Sequencial)")
    exit()
